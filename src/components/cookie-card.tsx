...
export function CookieCard({ cookie, eager = false }: { cookie: Cookie; eager?: boolean }) {
  const element = elementDefinitions[cookie.element];
  const role = roleDefinitions[cookie.role];
  ...
  return (
    <article className={`cookie-card rarity-${cookie.rarity.toLowerCase()}`}>
      ...
      <div className="cookie-card__body">
        ...
        <div className="cookie-card__taxonomy">
          <span><Image src={element.image} alt="" width={28} height={28} /><small>Element</small><strong>{element.label}</strong></span>
          <span><Image src={role.image} alt="" width={28} height={28} /><small>Role</small><strong>{role.label}</strong></span>
        </div>
